...
function AuthButton(){
    ...
    if(session){
        return <button onClick={()=>signOut()} className="hover:bg-blue-500 hover:text-white pr-2 bg-white dark:bg-black rounded-full px-2 py-1 space-x-2 p-1 flex items-center border-2 border-gray-300 dark:border-gray-800" ><span className="font-semibold">Sign Out</span></button>
    }
    else{
        return <button onClick={()=>signIn()} className="hover:bg-blue-600 hover:text-white pr-2 bg-blue-500 text-white dark:bg-black rounded-sm px-2 py-1 space-x-2 p-1 flex items-center border-2 border-gray-300 dark:border-gray-800" ><span className="font-semibold">Sign In with OAuth</span></button>
    }
}
